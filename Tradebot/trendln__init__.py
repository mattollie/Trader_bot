...
def plot_sup_res_learn(curdir, hist):
    ...
    def fig_slopeint():
        ...
        mvisual = (hist.Close.iloc[-3] - hist.Close.iloc[-1]) * drawdim[1] / yaxhgt / (-2 * drawdim[
            0] / xaxwdt)  # scale is 2:yaxhgt, could use this in computations, but must do dynamically with event handler since draw scale changes
        ...
        intcpt = ((hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2 - ax.get_ylim()[0]) * drawdim[1] / yaxhgt - (
                    -(drawdim[0] / 2) / mvisual)
        ann = plt.annotate(r'$d=\sqrt{{({}-{})^2+({}-{})^2}}={}$'.format(hist.Close.iloc[-3], hist.Close.iloc[-1], 0, 2,
                                                                         round(dist, 2)),
                           (len(hist.Close) - 2, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
                           ax.transData.inverted().transform(
                               ((drawdim[0] * 0.54) + bbox.x0, (-(drawdim[0] * 0.54) / mvisual + intcpt) + bbox.y0)),
                           textcoords='data', color='green', ha='center', va='center',
                           arrowprops={'arrowstyle': '-[', 'color': 'green'})
        # print(drawdim, ann.xyann, mvisual, intcpt, ax.get_xlim())
        plt.annotate(
            r'$b={}-{}*{}={}-{}*{}={}$'.format(hist.Close.iloc[-1], round(m, 2), 2, hist.Close.iloc[-3], round(m, 2), 0,
                                               b1), (len(hist.Close) - 3, hist.Close.iloc[-3]),
            (len(hist.Close) - 3, hist.Close.iloc[-3] - height * 0.1), arrowprops={'arrowstyle': '->'})
        plt.plot([len(hist.Close) - 2, len(hist.Close) - 2], [m * 1 + b1, hist.Close.iloc[-2]], 'r--')
        plt.annotate((r'$d=$' + '\n' + r'$\left|{}*{}+{}-{}\right|$' + '\n' + r'$={}$').format(round(m, 2), 1, b1,
                                                                                               hist.Close.iloc[-2],
                                                                                               round(d, 2)),
                     (len(hist.Close) - 2, (m * 1 + b1 + hist.Close.iloc[-2]) / 2),
                     (len(hist.Close) - 2 + 0.1, (m * 1 + b1 + hist.Close.iloc[-2]) / 2), va='center', color='red',
                     arrowprops={'arrowstyle': '-[', 'color': 'red'})
        plt.annotate(
            r'$m=\frac{{{}}}{{{}}}={}$'.format(round(hist.Close.iloc[-3] - hist.Close.iloc[-1], 2), 0 - 2, round(m, 2)),
            (len(hist.Close) - 2, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
            (len(hist.Close) - 2 + 0.2, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2 - height * 0.1), color='black',
            arrowprops={'arrowstyle': '->'})
        plt.plot([len(hist.Close) - 3, len(hist.Close) - 1], [hist.Close.iloc[-3], hist.Close.iloc[-3]], 'c--')
        plt.annotate(r'$\Delta x={}-{}={}$'.format(0, 2, 0 - 2), (len(hist.Close) - 2, hist.Close.iloc[-3]),
                     (len(hist.Close) - 2, hist.Close.iloc[-3] + height * 0.10), color='cyan', ha='center', va='center',
                     arrowprops={'arrowstyle': '-[', 'color': 'cyan'})
        plt.plot([len(hist.Close) - 1, len(hist.Close) - 1], [hist.Close.iloc[-3], hist.Close.iloc[-1]], 'c--')
        plt.annotate(r'$\Delta y={}-{}={}$'.format(hist.Close.iloc[-3], hist.Close.iloc[-1],
                                                   round(hist.Close.iloc[-3] - hist.Close.iloc[-1], 2)),
                     (len(hist.Close) - 1, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2),
                     (len(hist.Close) - 2 + 0.5, (hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2), color='cyan',
                     ha='center', va='center', arrowprops={'arrowstyle': '-[', 'color': 'cyan'})
        ...
        def redraw(event):
            ...
            intcpt = ((hist.Close.iloc[-3] + hist.Close.iloc[-1]) / 2 - ax.get_ylim()[0]) * drawdim[1] / yaxhgt - (
                        -(drawdim[0] / 2) / mvisual)
            # print(drawdim, ann.xyann, mvisual, intcpt, ax.get_xlim())
            ann.xyann = ax.transData.inverted().transform(
                ((drawdim[0] * 0.54) + bbox.x0, (-(drawdim[0] * 0.54) / mvisual + intcpt) + bbox.y0))
        ...
        plt.savefig(os.path.join(curdir, 'data', 'slopeint.svg'),
                    format='svg')  # , bbox_inches = extent, pad_inches = 0)
        plt.savefig(os.path.join(curdir, 'data', 'slopeint.png'),
                    format='png')  # , bbox_inches = extent, pad_inches = 0)
        # plt.show()
    ...
    def fig_linregrs():
        ...
        plt.gca().get_yaxis().set_visible(False)
        plt.annotate(
            r'Mean of n-Points along x and y-axes: $\bar{x}=\frac{\sum_{i=1}^n{x_i}}{n}, \bar{y}=\frac{\sum_{i=1}^n{y_i}}{n}$' + '\n' +
        ...
        plt.annotate(r'$\bar{{x}}=\frac{{{}+{}+{}}}{{{}}}={}$'.format(0, 1, 2, 3, 1),
                     (xbar + len(hist.Close) - 3, (hist.Close.iloc[-3:].min() + hist.Close.iloc[-3:].max()) / 2),
                     (xbar + len(hist.Close) - 3, hist.Close.iloc[-3:].min()), color='red', va='center',
                     arrowprops={'arrowstyle': '->', 'color': 'red'})
        plt.vlines(xbar + len(hist.Close) - 3, hist.Close.iloc[-3:].min(), hist.Close.iloc[-3:].max(), colors='r',
                   linestyles='--')
        plt.annotate(r'$\bar{{y}}=\frac{{{}+{}+{}}}{{{}}}={}$'.format(hist.Close.iloc[-3], hist.Close.iloc[-2],
                                                                      hist.Close.iloc[-1], 3, round(ybar, 2)),
                     (len(hist.Close) - 2, ybar), (len(hist.Close) - 1, ybar - height * 0.1), color='red', va='top',
                     ha='right', arrowprops={'arrowstyle': '->', 'color': 'red'})
        m = ((0 - xbar) * (hist.Close.iloc[-3] - ybar) + (1 - xbar) * (hist.Close.iloc[-2] - ybar) + (2 - xbar) * (
                    hist.Close.iloc[-1] - ybar)) / (np.square(0 - xbar) + np.square(1 - xbar) + np.square(2 - xbar))
        b = ybar - m * xbar
        SSR = np.square(hist.Close.iloc[-3] - (m * 0 + b)) + np.square(hist.Close.iloc[-2] - (m * 1 + b)) + np.square(
            hist.Close.iloc[-1] - (m * 2 + b))
        ...
        plt.annotate(r'$\hat{{y}}_0={}*{}+{}={}$'.format(round(m, 2), 0, round(b, 2), round(m * 0 + b, 2)),
                     (len(hist.Close) - 3, m * 0 + b), (len(hist.Close) - 3 + 0.1, m * 0 + b), va='top',
                     arrowprops={'arrowstyle': '->'})
        plt.annotate(r'$\hat{{y}}_1={}*{}+{}={}$'.format(round(m, 2), 1, round(b, 2), round(m * 1 + b, 2)),
                     (len(hist.Close) - 2, m * 1 + b), (len(hist.Close) - 2 + 0.15, m * 1 + b + height * 0.01),
                     arrowprops={'arrowstyle': '->'})
        plt.annotate(r'$\hat{{y}}_2={}*{}+{}={}$'.format(round(m, 2), 2, round(b, 2), round(m * 2 + b, 2)),
                     (len(hist.Close) - 1, m * 2 + b), (len(hist.Close) - 1 - 0.1, m * 2 + b + height * 0.1),
        ...
        plt.annotate(r'$y_0-\bar{{y}}={}$'.format(round(hist.Close.iloc[-3] - ybar, 2)),
                     (len(hist.Close) - 3, (hist.Close.iloc[-3] + ybar) / 2 + height * 0.1),
                     (len(hist.Close) - 3 + 0.1, (hist.Close.iloc[-3] + ybar) / 2 + height * 0.1), color='green',
                     va='center', arrowprops={'arrowstyle': '-[', 'color': 'green'})
        plt.annotate(r'$y_1-\bar{{y}}={}$'.format(round(hist.Close.iloc[-2] - ybar, 2)),
                     (len(hist.Close) - 2, (hist.Close.iloc[-2] + ybar) / 2 + height * 0.1),
                     (len(hist.Close) - 2 + 0.1, (hist.Close.iloc[-2] + ybar) / 2 + height * 0.1), color='green',
                     va='center', arrowprops={'arrowstyle': '-[', 'color': 'green'})
        plt.annotate(r'$y_2-\bar{{y}}={}$'.format(round(hist.Close.iloc[-1] - ybar, 2)),
                     (len(hist.Close) - 1, (hist.Close.iloc[-1] + ybar) / 2),
                     (len(hist.Close) - 1 - 0.1, (hist.Close.iloc[-1] + ybar) / 2), color='green', va='center',
                     ha='right', arrowprops={'arrowstyle': '-[', 'color': 'green'})
        ...
        plt.annotate(r'$y_0-\hat{{y}}={}$'.format(round(hist.Close.iloc[-3] - (m * 0 + b), 2)),
                     (len(hist.Close) - 3, (hist.Close.iloc[-3] + m * 0 + b) / 2),
                     (len(hist.Close) - 3 + 0.1, (hist.Close.iloc[-3] + m * 0 + b) / 2), color='cyan', va='center',
                     arrowprops={'arrowstyle': '-[', 'color': 'cyan'})
        plt.annotate(r'$y_1-\hat{{y}}={}$'.format(round(hist.Close.iloc[-2] - (m * 1 + b), 2)),
                     (len(hist.Close) - 2, (hist.Close.iloc[-2] + m * 1 + b) / 2),
                     (len(hist.Close) - 2 + 0.1, (hist.Close.iloc[-2] + m * 1 + b) / 2), color='cyan', va='center',
                     arrowprops={'arrowstyle': '-[', 'color': 'cyan'})
        plt.annotate(r'$y_2-\hat{{y}}={}$'.format(round(hist.Close.iloc[-1] - (m * 2 + b), 2)),
                     (len(hist.Close) - 1, (hist.Close.iloc[-1] + m * 2 + b) / 2 - height * 0.05),
                     (len(hist.Close) - 1 - 0.1, (hist.Close.iloc[-1] + m * 2 + b) / 2 - height * 0.05), color='cyan',
                     va='center', ha='right', arrowprops={'arrowstyle': '-[', 'color': 'cyan'})
        plt.annotate(
            (r'$m=\frac{{({}-{})*{}+({}-{})*{}+({}-{})*{}}}{{({}-{})^2+({}-{})^2+({}-{})^2}}$' + '\n' + '=${}$' + '\n' +
             r'$SSR={}^2+{}^2+{}^2={}$' + '\n' +
             r'$\sigma_m=\sqrt{{\frac{{{}}}{{({}-2)(({}-{})^2+({}-{})^2+({}-{})^2)}}}}={}$' + '\n' +
             r'$\sigma_b={}\sqrt{{\frac{{{}^2+{}^2+{}^2}}{{{}}}}}={}$'
             ).format(0, round(xbar, 2), round(hist.Close.iloc[-3] - ybar, 2), 1, round(xbar, 2),
                      round(hist.Close.iloc[-2] - ybar, 2), 2, round(xbar, 2), round(hist.Close.iloc[-1] - ybar, 2), 0,
                      round(xbar, 2), 1, round(xbar, 2), 2, round(xbar, 2), round(m, 2),
                      round(hist.Close.iloc[-3] - (m * 0 + b), 2), round(hist.Close.iloc[-2] - (m * 1 + b), 2),
                      round(hist.Close.iloc[-1] - (m * 2 + b), 2), round(SSR, 2),
                      round(SSR, 2), 3, 0, round(xbar, 2), 1, round(xbar, 2), 2, round(xbar, 2), round(err1, 2),
                      round(err1, 2), 0, 1, 2, 3, round(err2, 2)),
            (len(hist.Close) - 2, m * 1 + b), (len(hist.Close) - 1, hist.Close.iloc[-3:].min()), color='blue',
            va='bottom', ha='right', arrowprops={'arrowstyle': '->', 'color': 'blue'})
        plt.annotate(r'$b={}-{}*{}={}$'.format(round(ybar, 2), round(m, 2), xbar, round(b, 2)),
                     (len(hist.Close) - 3, b), (len(hist.Close) - 3 + 0.1, b), color='blue', ha='left',
        ...
        plt.tight_layout()
        # extent = plt.gcf().get_window_extent(renderer=plt.gcf().canvas.get_renderer()).transformed(plt.gcf().dpi_scale_trans.inverted())
        plt.savefig(os.path.join(curdir, 'data', 'linregrs.svg'),
                    format='svg')  # , bbox_inches = extent, pad_inches = 0)
        plt.savefig(os.path.join(curdir, 'data', 'linregrs.png'),
                    format='png')  # , bbox_inches = extent, pad_inches = 0)
    ...
    def fig_hough():
        ...
        plt.annotate(r'Slope of Perpendicular Line: $m_p=-\frac{1}{m}, mm_p=-1$' + '\n' +
                     ...
                     r'$\rho=x \cos \theta+y \sin \theta$ where $\sin \theta=\frac{opposite}{hypotenuse}, \cos \theta=\frac{adjacent}{hypotenuse}$ and $y=\frac{\sin \theta}{\cos \theta}x$ while its perpendicular line is $y=-\frac{\cos \theta}{\sin \theta}x+\frac{\rho}{\sin \theta}$',
                     (0, 0))
        plt.subplot(spec[0, 0])
        ...
        plt.annotate(r'Diagonal length=$\sqrt{{{}^2+{}^2}}={}$'.format(9, round(mx - mn, 2),
                                                                       round(np.sqrt(np.square(9) + np.square(mx - mn)),
                                                                             2)), (len(hist.Close) - 1, mx),
                     (len(hist.Close) - 1 - 1, mx), ha='right', va='top', color='blue',
                     arrowprops={'arrowstyle': '->', 'color': 'blue'})
        # plt.xlim(0, 30)
        # plt.ylim(0, 30)
        ...
        plt.annotate(r'$y={}x+{}$'.format(round(m, 2), round(b, 2)), (len(hist.Close) - 5.5, (mn + mx) / 2),
                     (len(hist.Close) - 5.5, mn + (mx - mn) * 0.7), arrowprops={'arrowstyle': '->'})
        ...
        plt.annotate('', (len(hist.Close) - 10, mn), (len(hist.Close) - 10 + x0, mn + -x0 / m + bperp),
                     arrowprops=dict(arrowstyle="<|-", color='red'))
        plt.gca().add_patch(
            mpatches.Wedge((len(hist.Close) - 10 + x0, mn + -x0 / m + bperp), 1, angle * 180 / np.pi - 180,
                           angle * 180 / np.pi - 90, fill=False))
        plt.gca().add_patch(
            mpatches.Wedge((len(hist.Close) - 10 + x0, mn + -x0 / m + bperp), 0.5, angle * 180 / np.pi - 270,
                           angle * 180 / np.pi - 180, fill=False))
        ...
        plt.annotate((
                    r'$\rho=\frac{{\left|{}+{}*{}-{}\right|}}{{\sqrt{{1 + {}^2}}}}$' + '\n' + '$={}\cos {}+{}\sin {}$' + '\n' + '$={}\cos {}+{}\sin {}$' + '\n' + '$={}$').format(
            round(b, 2), round(m, 2), 0, 0, round(m, 2),
            0, round(angle * 180 / np.pi, 2), round(hist.Close.iloc[-10] - mn, 2), round(angle * 180 / np.pi, 2), 9,
            round(angle * 180 / np.pi, 2), hist.Close.iloc[-1] - mn, round(angle * 180 / np.pi, 2),
            round(0 * np.cos(angle) + (hist.Close.iloc[-10] - mn) * np.sin(angle), 2)),
            (len(hist.Close) - 10 + x0 / 2, mn + (-x0 / m + bperp) / 2),
            (len(hist.Close) - 10 + x0 / 2, mn + (-x0 / m + bperp) / 2 + 0.9), ha='center', color='red',
            arrowprops=dict(arrowstyle="->", color='red'))
        plt.plot([len(hist.Close) - 10, len(hist.Close) - 1], [mn, mn], 'k-')
        plt.plot([len(hist.Close) - 10, len(hist.Close) - 10], [mn, mx], 'k-')
        plt.annotate('{}'.format(9), (len(hist.Close) - 5.5, mn), (len(hist.Close) - 5.5, mn + 0.5), ha='center',
                     arrowprops=dict(arrowstyle="->"))
        plt.annotate('{}'.format(round(mx - mn, 2)), (len(hist.Close) - 10, (mn + mx) / 2),
                     (len(hist.Close) - 10 + 0.5, (mn + mx) / 2), ha='left', arrowprops=dict(arrowstyle="->"))
    ...
    def fig_minima():
        ...
        for x in range(len(h)):
            ann = plt.gca().annotate('{}\n'.format(round(mom[x], 2), round(momacc[x], 2)), (x, h.Close.iloc[x]),
                                     (x, h.Close.iloc[x] - (h.Close.max() - h.Close.min()) / 3),
                                     ha='center')  # bbox=dict(boxstyle='round', fc='gray', alpha=0.3)
            # plt.rcParams['font.size']
            ext = ann.get_window_extent(renderer=plt.gcf().canvas.get_renderer()).transformed(
                plt.gca().transData.inverted())
            ann = plt.gca().annotate('{}'.format(round(momacc[x], 2)), (x, ext.y0), ha='center', va='bottom', color='r')
            next = ann.get_window_extent(renderer=plt.gcf().canvas.get_renderer()).transformed(
                plt.gca().transData.inverted())
            patch = plt.gca().add_artist(
                mpatches.FancyBboxPatch((min(ext.x0, next.x0) + 0.1, ext.y0), width=max(ext.width, next.width) - 0.2,
                                        height=ext.height + 3, boxstyle='round', fc='gray', alpha=0.3))
            plt.gca().annotate(''.format(round(mom[x], 2), round(momacc[x], 2)), (x, h.Close.iloc[x]), (x, ext.y1 + 3),
                               arrowprops={'arrowstyle': '-|>'})
        plt.ylim(h.Close.min() - (h.Close.max() - h.Close.min()) / 2.8,
                 h.Close.max() + (h.Close.max() - h.Close.min()) / 10)
    ...
    def fig_reimann():
        ...
        for x, y in enumerate(ser):
            plt.bar(x, (m * (x + base) + b) - y if isMin else y - (m * (x + base) + b),
                    color='r' if (y < (m * (x + base) + b) if isMin else y > (m * (x + base) + b)) else 'gray')

        plt.annotate(r'S={}, $\frac{{{}}}{{{}}}$={}$\frac{{\$}}{{day}}$'.format(round(S, 2), round(S, 2),
                                                                                len(range(base, trendline[0][-1] + 1)),
                                                                                round(area,
                                                                                      2)) + '\n' + r'Reimann Sum where $\Delta x=x_i-x_{i-1}$,' + '\n' + '$x_i^* \in [x_{i-1}, x_i]$: $S=\sum_{i=1}^n{f(x_i^*)\Delta x_i}$',
                     (0, plt.gca().get_ylim()[0] + 5), va='bottom')
        plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(6))
        # plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
        plt.gca().xaxis.set_major_formatter(ticker.FuncFormatter(datefmt(hist[-250:].index)))
        plt.setp(plt.gca().get_xticklabels(), rotation=30, ha='right')
...
def test_sup_res(curdir):
    ...
    data = [float(x) for x in data]
    result = (([], [np.nan, np.nan], [], [[]]), ([3, 9, 15, 21], [0.16666666666666677, 2.499999999999998],
                                                 [([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))],
                                                 [[([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))]]))
    assert result == calc_support_resistance(data, extmethod=METHOD_NAIVE)
    result = (([7, 13, 19], [0.1666666666666666, -0.1666666666666652],
               [([7, 13, 19], (0.16666666666666666, -0.16666666666666652, 0.0, 0.0, 0.0, 0.0))],
               [[([7, 13, 19], (0.16666666666666666, -0.16666666666666652, 0.0, 0.0, 0.0, 0.0))]]), (
              [3, 9, 15, 21], [0.16666666666666677, 2.499999999999998],
              [([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))],
              [[([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))]]))
    assert result == calc_support_resistance(data, extmethod=METHOD_NAIVECONSEC)
    result = (([23], [np.nan, np.nan], [], [[]]), ([3, 9, 15, 21], [0.16666666666666677, 2.499999999999998],
                                                   [([3, 9, 15, 21], (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))], [
                                                       [([3, 9, 15, 21],
                                                         (0.16666666666666666, 2.5, 0.0, 0.0, 0.0, 0.0))]]))
    assert result == calc_support_resistance(data)
    assert result == calc_support_resistance(data, method=METHOD_NCUBED)
    assert result == calc_support_resistance(data, method=METHOD_HOUGHPOINTS)
    assert result == calc_support_resistance(data, method=METHOD_HOUGHLINES)
...
def calc_support_resistance(h, extmethod=METHOD_NUMDIFF, method=METHOD_NSQUREDLOGN,
    ...
    def get_trend(Idxs, h, fltpct, min_h, max_h):
        trend = []
        for x in range(len(
                Idxs)):  # unfortunately an O(n(n-1)(n-2))=O((n^2-n)(n-2))=O(n^3-3n^2-2n)~=O(n^3) algorithm but meets the strict definition of a trendline
    ...
    def prob_hough(Idxs, h, fltpct, min_h, max_h):
        ...
        for x in range(hough_prob_iter):
            lines.extend(probabilistic_hough_line(image, threshold=2, theta=tested_angles, line_length=0,
                                                  line_gap=int(np.ceil(
                                                      np.sqrt(np.square(image.shape[0]) + np.square(image.shape[1]))))))
        l = []
    ...
    def calc_all(idxs, h, isMin):
        ...
        mtrend = [(pts, (res[0], res[1], res[2], res[3], res[4], measure_area((pts, res), isMin, h))) for pts, res in
                  mtrend]
    ...
    # pip install scikit-image
        ...
        raise ValueError(
            'method must be one of METHOD_NCUBED, METHOD_NSQUREDLOGN, METHOD_HOUGHPOINTS, METHOD_HOUGHLINES, METHOD_PROBHOUGH')
...
def plot_sup_res_date(hist, idx, numbest=2, fromwindows=True, pctbound=0.1,
    ...
    return plot_support_resistance(hist, ticker.FuncFormatter(datefmt(idx)), numbest, fromwindows,
                                   pctbound, extmethod, method, window, errpct, hough_scale, hough_prob_iter, sortError,
                                   accuracy)
...
def plot_support_resistance(hist, xformatter=None, numbest=2, fromwindows=True,
                            ...
                            window=125, errpct=0.005, hough_scale=0.01, hough_prob_iter=10, sortError=False,
                            accuracy=1):
    import matplotlib.pyplot as plt
    import matplotlib.ticker as ticker
    ret = calc_support_resistance(hist, extmethod, method, window, errpct, hough_scale, hough_prob_iter, sortError,
                                  accuracy)
    plt.clf()
    plt.subplot(111)
    if len(ret) == 2:
        ...
        if type(hist) is tuple and len(hist) == 2 and check_num_alike(hist[0]) and check_num_alike(hist[1]):
            ...
            disp = [(hist[0], minimaIdxs, pmin, 'yo', 'Avg. Support', 'y--'),
                    (hist[1], maximaIdxs, pmax, 'bo', 'Avg. Resistance', 'b--')]
            ...
            disp = [(hist, minimaIdxs, pmin, 'yo', 'Avg. Support', 'y--'),
                    (hist, maximaIdxs, pmax, 'bo', 'Avg. Resistance', 'b--')]
        ...
        disp = [(hist[1], maximaIdxs, pmax, 'bo', 'Avg. Resistance', 'b--') if hist[0] is None else (
        hist[0], minimaIdxs, pmin, 'yo', 'Avg. Support', 'y--')]
        dispwin = [
            (hist[1], maxwindows, 'Resistance', 'r--') if hist[0] is None else (hist[0], minwindows, 'Support', 'g--')]
        disptrend = [
            (hist[1], maxtrend, 'Resistance', 'r--') if hist[0] is None else (hist[0], mintrend, 'Support', 'g--')]
        plt.plot(range(len_h), hist[1 if hist[0] is None else 0], 'k--',
                 label=('High' if hist[0] is None else 'Low') + ' Price')
